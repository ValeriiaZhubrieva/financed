(() => {
    ...
    function rangeInit() {
        const rangeDeadline = document.querySelectorAll(".range-deadline");
        const rangeSlide = document.querySelectorAll(".range-slide");
        if (rangeSlide.length) rangeSlide.forEach((range => {
            let minValue = parseInt(range.getAttribute("data-min"));
            let maxValue = parseInt(range.getAttribute("data-max"));
            initialize(range, {
                start: minValue,
                connect: "lower",
                range: {
            ...
            const inputValue = range.parentElement.querySelector(".range-input");
            if (inputValue) {
                function setValues() {
                    let inputStartValue = parseInt(inputValue.value, 10);
                    if (isNaN(inputStartValue)) inputStartValue = minValue;
                    inputStartValue = Math.max(minValue, Math.min(inputStartValue, maxValue));
                    range.noUiSlider.set([ inputStartValue ]);
                }
                range.noUiSlider.on("update", (function(values, handle) {
                    const value = parseInt(values[handle], 10);
                    if (!isNaN(value)) inputValue.value = value;
                }));
                inputValue.addEventListener("change", setValues);
            }
        }));
        if (rangeDeadline.length) rangeDeadline.forEach((range => {
            let minValue = parseInt(range.getAttribute("data-min"));
            let maxValue = parseInt(range.getAttribute("data-max"));
            initialize(range, {
                start: minValue,
            ...
            let rangeSliderValueElement = range.parentElement.querySelector(".range-value");
            range.noUiSlider.on("update", (function(values, handle) {
    ...
    function initSliders() {
        ...
        resizableSwiper("(max-width: 767.98px)", ".partners__slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
        ...
        if (document.querySelector(".how-it-works__slider")) new Swiper(".how-it-works__slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
        ...
        if (document.querySelector(".services__slider")) {
            const servicesSlider = document.querySelectorAll(".services__slider");
            servicesSlider.forEach((slider => {
                const prevBtn = slider.parentElement.querySelector(".swiper-button-prev");
                const nextBtn = slider.parentElement.querySelector(".swiper-button-next");
                new Swiper(slider, {
